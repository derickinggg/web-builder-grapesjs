...
describe('Collection component getters and setters', () => {
  ...
  describe('Getters', () => {
    ...
    test('getComponentDef should return the correct component definition', () => {
      ...
      expect(componentDef.attributes?.['data-user']).toEqual({
        type: DataCollectionVariableType,
        variableType: DataCollectionStateVariableType.currentItem,
        collectionId: 'my_collection',
        path: 'user',
  ...
  describe('Setters', () => {
    ...
    test('setStartIndex should update the start index and reflect in children', () => {
      ...
      expect(children.at(0).getAttributes()['data-user']).toBe('user1');
      expect(children.at(1).getAttributes()['data-user']).toBe('user2');
      expect(children.at(2).getAttributes()['data-user']).toBe('user3');
    });
    ...
    test('setEndIndex should update the end index and reflect in children', () => {
      ...
      const children = cmp.components();
      expect(children).toHaveLength(2);
      expect(children.at(0).getAttributes()['data-user']).toBe('user2');
      expect(children.at(1).getAttributes()['data-user']).toBe('user3');
    });
    ...
    test('setDataSource should update the data source and reflect in children', () => {
      ...
      const children = cmp.components();
      expect(children).toHaveLength(1); // Only one record in the new data source
      expect(children.at(0).getAttributes()['data-user']).toBe('user4');
    });

    test('setStartIndex with zero should include the first record', () => {
      cmp.setStartIndex(0);
      const children = cmp.components();
      expect(children).toHaveLength(3); // Includes all records
      expect(children.at(0).getAttributes()['data-user']).toBe('user1');
    });
    ...
    test('setEndIndex with zero should result in no children', () => {
      cmp.setEndIndex(0);
      const children = cmp.components();
      expect(children).toHaveLength(0); // No records included
    });

    test('setDataSource with an empty data source should result in no children', () => {
      const emptyDataSource = dsm.add({
      ...
      const children = cmp.components();
      expect(children).toHaveLength(0); // No records in the data source
    });
  });
  ...
  describe('Impact on HTML output', () => {
    ...
    test('HTML output should reflect changes in startIndex', () => {
      ...
      expect(html).toContain('data-user="user1"');
      expect(html).toContain('data-user="user2"');
      expect(html).toContain('data-user="user3"');
    });
    ...
    test('HTML output should reflect changes in dataSource', () => {
      ...
      const html = cmp.toHTML();
      expect(html).toContain('data-user="user4"');
    });
